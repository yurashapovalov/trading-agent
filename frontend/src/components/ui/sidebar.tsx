...
function useSidebar(side: SidebarSide = "left") {
  ...
  return {
    ...
    toggleSidebar: sideState.toggle,
    isMobile: context.isMobile,
    width: sideState.width,
    setWidth: sideState.setWidth,
    minWidth: sideState.minWidth,
    maxWidth: sideState.maxWidth,
    isDragging: sideState.isDragging,
    setIsDragging: sideState.setIsDragging,
  }
}
...
function SidebarProvider({
  ...
  const [rightIsDragging, setRightIsDragging] = React.useState(false)

  // Cookie helpers
  const setLeftOpen = (value: boolean | ((v: boolean) => boolean)) => {
    const newState = typeof value === "function" ? value(leftOpen) : value
    setLeftOpenState(newState)
    document.cookie = `sidebar_left=${newState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
  }

  const setRightOpen = (value: boolean | ((v: boolean) => boolean)) => {
    const newState = typeof value === "function" ? value(rightOpen) : value
    setRightOpenState(newState)
    document.cookie = `sidebar_right=${newState}; path=/; max-age=${SIDEBAR_COOKIE_MAX_AGE}`
  }

  // Toggle helpers
  const toggleLeft = () => {
    isMobile ? setLeftOpenMobile((v) => !v) : setLeftOpen((v) => !v)
  }

  const toggleRight = () => {
    isMobile ? setRightOpenMobile((v) => !v) : setRightOpen((v) => !v)
  }

  // Keyboard shortcut for left sidebar (Cmd+B)
  React.useEffect(() => {
  ...
  const contextValue: SidebarContextProps = {
    left: {
      ...
      setIsDragging: setLeftIsDragging,
    },
    right: {
      ...
      setIsDragging: setRightIsDragging,
    },
    isMobile,
  }
...
function Sidebar({
  ...
  const { isMobile, state, openMobile, setOpenMobile, width, isDragging } = useSidebar(side)
...
function SidebarRail({
  ...
  const { handleMouseDown } = useSidebarResize({
    ...
    cookieName: `sidebar_${side}_width`,
  ...
  return (
    <button
      ...
      onClick={enableDrag ? undefined : toggleSidebar}
