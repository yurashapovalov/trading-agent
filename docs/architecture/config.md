...
User: "что было в день OPEX?"
      ↓
Parser: RAP добавляет события из config/market/events.py
      ↓
Planner: находит даты OPEX через Date Resolver
      ↓
Executor: фильтрует данные по этим датам
```
...
RAP включает события в промпт парсера для правильной интерпретации.
...
|-----------|----------------------|
| **Planner** | Праздники — Date Resolver пропускает при расчёте дат |
| **Executor** | Инструменты — сессии, торговые дни |
| **Scanner** | Паттерны — параметры детекции (is_doji, is_hammer...) |
| **RAP** | Паттерны, праздники, события — контекст для промпта |
